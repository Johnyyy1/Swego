...
- `source_relationships` is a rebuildable, repository-scoped projection of statically resolved imports and re-exports between versioned source documents. It retains binding names/kinds, relative/alias/baseUrl provenance, verified exact-symbol kind/range or explicit module-only state, config/source/target revisions, temporal validity, parser provenance, line, reason, and confidence.
