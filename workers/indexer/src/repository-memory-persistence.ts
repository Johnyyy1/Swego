    ...
    await transaction
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          importedName: sql`excluded.imported_name`,
          localName: sql`excluded.local_name`,
          exposedName: sql`excluded.exposed_name`,
          bindingKind: sql`excluded.binding_kind`,
          isTypeOnly: sql`excluded.is_type_only`,
          resolution: sql`excluded.resolution`,
          moduleResolutionKind: sql`excluded.module_resolution_kind`,
          targetSymbolKind: sql`excluded.target_symbol_kind`,
          targetStartLine: sql`excluded.target_start_line`,
          targetEndLine: sql`excluded.target_end_line`,
          configurationPath: sql`excluded.configuration_path`,
          configurationCommitSha: sql`excluded.configuration_commit_sha`,
          availableAt: sql`excluded.available_at`,
